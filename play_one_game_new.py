...
def get_number_simulations():
    return int(input("Enter the number of simulations: "))

def main():
    count=0
    simulation_game = get_number_simulations()
    Output_Simulation = []
    ...
    bingo_counts=0
    full_house_counts=0
     # Count the number of Bingos and Full-Houses
    bingo_counts += len([item for item in Output_per_Simulation if item[0] == 'B'])
    full_house_counts += len([item for item in Output_per_Simulation if item[0] == 'FH'])

    
    print("Bingo Counts:", bingo_counts)
    print("Full House Counts:", full_house_counts)
